...
class Preprocessing:
    ...
    def _convert_datatypes(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Przebieg'] = df['Przebieg'].astype(str).str.replace(' km', '').str.replace(' ', '').apply(lambda x: self._convert_to_type(x, float))
        df['Pojemność skokowa'] = df['Pojemność skokowa'].str.replace(' cm3', '').str.replace(' ', '').apply(lambda x: self._convert_to_type(x, float))
        df['Moc'] = df['Moc'].str.replace(' KM', '').str.replace(' ', '').apply(lambda x: self._convert_to_type(x, float))
        df['Spalanie W Mieście'] = df['Spalanie W Mieście'].str.replace(' l/100km', '').str.replace(',', '.').str.replace(' ', '').apply(lambda x: self._convert_to_type(x, float))
